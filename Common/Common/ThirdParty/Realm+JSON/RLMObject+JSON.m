...
+ (BOOL)values:(RLMObject*)object1 isEqual:(RLMObject*)object2 {
    return [[[object1 JSONDictionary] mj_JSONString] isEqualToString:[[object2 JSONDictionary] mj_JSONString]];
}
+ (BOOL)arrayValues:(NSArray<RLMObject*>*)array1 isEqual:(NSArray<RLMObject*>*)array2 {
    ...
    if(array1.count != array2.count)
        return NO;
    //全部弄成字符串数组，然后再来比较
    NSMutableArray<NSString*> *leftArray = [@[] mutableCopy];
    for (RLMObject *tempRLMObject in array1) {
        [leftArray addObject:[[tempRLMObject JSONDictionary] mj_JSONString]];
    }
    NSMutableArray<NSString*> *rightArray = [@[] mutableCopy];
    for (RLMObject *tempRLMObject in array2) {
        [rightArray addObject:[[tempRLMObject JSONDictionary] mj_JSONString]];
    }
    //依次进行寻找，发现一个不一样的就退出
    for (int index = 0; index < leftArray.count; index ++) {
        NSString *leftString = leftArray[index];
        //是否有与之对应的值
        BOOL haveThisValue = NO;
        for (NSString *tempString in rightArray) {
            if([tempString isEqualToString:leftString]) {
                haveThisValue = YES;
                break;
            }
        }
