...
- (Chater*)chaterWithImid:(int64_t)imid {
    ...
    chater.imid = imid;
    RLMRealm *rlmRealm = [self currThreadRealmInstance];
    RLMResults *results = [Chater objectsInRealm:rlmRealm withPredicate:[NSPredicate predicateWithFormat:[NSString stringWithFormat:@"imid = %@",@(imid)]]];
    if(results.count) {
        chater = [results[0] deepCopy];
    }
    return chater;
}
